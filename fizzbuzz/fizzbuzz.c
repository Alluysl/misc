...
/* Rules of the game:
   ...
   If the value is a multiple of 3 say Fizz, of 5 say Buzz, of 15 say FizzBuzz, else say the number */

#define ERROR_HANDLING_PAIR_FILE { perror("Error handling pairs file"); exit(EXIT_FAILURE); }
...
char* read_line(FILE* f){
	...
	while (size <= 0){
		...
		if (fgetpos(f, &pos)) ERROR_HANDLING_PAIR_FILE /* save the position of the descriptor */
		
		do { ++size; c = fgetc(f); } while (c != EOF && c != '\n');
		
		if (!size){
			if (c == EOF) /* reached end of file at beginning of line, nothing to read anymore */
				return NULL;
			size = -1; /* skip empty lines */
		} else
			if (fsetpos(f, &pos)) ERROR_HANDLING_PAIR_FILE /* restore the position of the descriptor if we keep going */
	}
	
	++size; /* amount of characters -> size of string */
	
	/* Allocate the memory for the string, sizeof(char) == 1 by definition */
...
intStringPair get_pair_from_line(char* line){
	...
	/* Get value */
	if ((sscanfRet = sscanf(line, "%d", &pair.i)) != 1 || strchr(line, ' ') == NULL){
		fprintf(stderr, "Couldn't parse line \"%s\".\n", line);
		if (sscanfRet == EOF) perror("The encountered error was");
		exit(EXIT_FAILURE);
	}
	...
	{
		int i,
		    diff; /* index of the part to bring to the beginning of the string */
...
intStringPairArray get_pairs(intStringPairArray arr, const char* const pairsFilePath){
	...
	while ((readLine = read_line(f)) != NULL){
		arr = intStringPairArray_append(arr, get_pair_from_line(readLine));
...
int main(){
	...
	while (fflush(stdout), /* make sure the line before is printed */
		...
		printf("Wrong number %d provided, should be 1 or above (15 or above recommended). Try again: ", n);
	}
	
	for (i = 1; i <= n; ++i){
		char special = 0; /* whether the value is "special" (doesn't show the number but text instead) */
	    	int j;
	    	
	    	for (j = 0; j < (int)specialValues.size; ++j){
			special |= check_print_special(i, specialValues.values[j].i, specialValues.values[j].s);
			if (i == n) /* last pass */
				free(specialValues.values[j].s); /* free allocated memory */
		}
